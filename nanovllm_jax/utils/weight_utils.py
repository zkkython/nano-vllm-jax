...
class WeightLoader:
    ...
    def load_weights_from_safetensors(
        ...
        # Determine expected prefixes to reduce noisy warnings when multiple loaders read the same files
        if hasattr(self.model, "blocks") or hasattr(self.model, "patch_embed"):
            expected_prefixes = ("visual.",)
        else:
            expected_prefixes = ("model.", "lm_head", "embed_tokens", "norm")

        logger.info(
            "WeightLoader target module: %s, expected_prefixes=%s",
        ...
        for hf_key, hf_weight in self._iterate_weights():
            if hf_key in regular_mappings:
                ...
                if self._is_excluded_layer_weight(hf_key):
                    ...
                    # Only warn when the key matches expected prefixes for this loader; otherwise, it belongs to another loader
                    if hf_key.startswith(expected_prefixes):
                        logger.warning(
                            "No mapping found for weight: %s (module=%s)",
                            hf_key,
                            type(self.model).__name__,
                        )
                    else:
                        logger.debug(
                            "Skipping irrelevant weight for this module: %s (expected_prefixes=%s)",
                            hf_key,
                            expected_prefixes,
                        )
